 ...
 *           
 *      Name:               The unique identifier/slug of the style variation. Required.
 *      Block Name:         The block name of the block type where the style variation is applied. Must include namespace and block slug (e.g. namespace/block-slug). Required
 *      Class Name:         The CSS Class Name generated by the style variations API for each specific variation (e.g., .is-style-{name}). Required
 ...
 * 
 * Uses the following custom function:
 * dynamically_enqueue_custom_block_style()
 * 
 */
function enqueue_block_style_variations_styles() {
    ...
    foreach( $style_variations_default_css_file_paths as $i => $file_path ) {
        ...
        // Create asset handle
        $handle = 'basse-block-style-variation---' . str_replace('/', '-', $css_file_headers['block_name']) . '---' . $css_file_headers['name'];
...
add_action( 'init', THEME_NS . '\enqueue_block_style_variations_styles' );
